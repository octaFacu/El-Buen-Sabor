...
export default function MisFavoritosComponent({ usuario }: Props) {
  const [productos, setProductos] = useState<PageProyeccionHistorialPedido<proyeccionProductoFavorito>>();
  ...
  const traerFavorito = async (pageNumber: number) => {
    ...
    const prod = await servicioFavorito.getFavoritosDeUsuario(
      await servicioCliente.getIdCliente(usuario.id), 5, pageNumber
    );
    setProductos(prod);
    ...
    <div className="card card-principal-favoritos card-altura scrollable-container-favoritos">
      <div className="card-body">
        {productos?.content?.length === 0 ? (
          <h2 className="text-white">Todavía no has ingresado ningún favorito.</h2>
        ) : (
                  ...
                  <div className="iconos">
                    ...
                    >
                      <i className="material-icons iconos">
                        {hoveredProductId === producto.id ? "favorite_border" : "favorite"}
                      </i>
                    </button>
                    <button className="SeparacionEntreIconos"
                      onMouseEnter={() => setHoverCartIcon(producto.id)}
                      onMouseLeave={() => setHoverCartIcon(null)}
                      onClick={() => console.log(producto)}
                    >
                      <i className={`material-icons iconos ${hoverCartIcon === producto.id ? "brillo" : ""}`}>
                        add_shopping_cart
                      </i>
                    </button>
                  </div>
        ...
        <Paginacion page={productos?.pageable.pageNumber} setPage={actualizarPagina} totalPages={productos?.totalPages} size={productos?.size} />
      </div>
    </div>
  </div>
