...
export default function OrdenamientosClienteComponent({ actualizarCriteriosOrdenamiento }: Props) {
    ...
    const handleResetFiltro = () => {
        ...
        actualizarCriteriosOrdenamiento("id_cliente", "DESC", null, null);
    }

    const handleBuscarConFechas = () => {
        if (startDate && endDate) {
            actualizarCriteriosOrdenamiento("id_cliente", switchActivo ? "DESC" : "ASC", startDate, endDate);
            setTituloDrop("Ordenar Por")
            setDesabilitado(true)
        }
        else if (startDate && endDate && startDate > endDate) {
    ...
    return (
        <>
            <div className="d-flex justify-content-between">
                <div>
                    <DatePicker selected={startDate} onChange={handleFechaInicio} placeholderText='fecha Inicio' dateFormat="yyyy-MM-dd" className="custom-datepicker" />
                    <DatePicker selected={endDate} onChange={handleFechaFin} placeholderText='fecha Fin' dateFormat="yyyy-MM-dd" className="custom-datepicker" />
