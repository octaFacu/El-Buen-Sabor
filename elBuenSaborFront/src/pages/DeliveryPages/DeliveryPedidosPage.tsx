...
export const DeliveryPedidosPage = () => {
  ...
  const handleOptionChange = () => {
    setEsDeliveryPropio(!esDeliveryPropio);
  };
    ...
    if (!esDeliveryPropio) {
      ...
      if (!isLoading) {
        ...
        if (user) {
          ...
          if(userId != undefined){
            servicePedido.getByDelivery(userId, rol)
              .then(data => {
                // Filter the data where esEnvio is true
                const filteredPedidos: Pedido[] = data.filter((pedido: Pedido) => pedido.esEnvio === true && pedido.estado === "EnDelivery");
                setPedidos(filteredPedidos);
              });
            }
        }
  ...
  useEffect(() => {
    ...
    const handleNotification = async (attributeValue: string) => {
      ...
      if (attributeValue === "Listo" || attributeValue === "EnDelivery") {
          ...
          {!esDeliveryPropio ? (
            <div>
              <div className="titleAndAddButton pb-3">
                ...
                <div className="px-3">
                  <div className="btn btn-success" onClick={handleOptionChange}> <span className="circleButtonText">
                     Pedidos Asignados
                  </span></div>
                </div>
              </div>
